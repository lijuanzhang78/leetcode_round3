...
public class MaximumAverageSubarrayII {
	public double findMaxAverage(int[] nums, int k) {
        ...
        while (min < max - 0.00001) { //binary search to find the max average between min and max
            double mid = min +(max-min)/2.0;
            if (hasAvgAbove(nums, k, mid)) {
                //min = mid + 0.00_005; //error less than 10-5 will be accepted.
                min = mid;
            } else max = mid; 
            //else max = mid - 0.00_005;
        }
        return max;
    }
